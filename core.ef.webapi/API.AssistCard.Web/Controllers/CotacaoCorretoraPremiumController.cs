...
namespace API.Viagem.Web.Controllers
{
    ...
    public class CotacaoCorretoraPremiumController : Controller
    {
        private readonly ICotacaoService _cotacaoSevice;

        public CotacaoCorretoraPremiumController(ICotacaoService cotacaoSevice)
        {
        ...
        public async Task<object> Post([FromBody] CotacaoEnvioDTO objetoCotacao)
        {
            try
            {
                ...
                {
                    object retorno = _cotacaoSevice.SalvarCotacao(objetoCotacao, OrigemParceiroEnum.CorretoraPremium);
